...
## Critérios de aceite
- [ ] Tela 4D lista os `order_item` com `kitchen_status = delivered`.
- [ ] Se `order.payment_status === "unpaid"`, a tela exibe um aviso destacado "Comanda ainda não paga".
- [ ] Se `order.payment_status === "paid"`, o aviso não é exibido.
- [ ] O aviso desaparece em tempo real assim que a comanda é fechada em outra tela (evento `order.updated` recebido via WebSocket), sem precisar recarregar.
