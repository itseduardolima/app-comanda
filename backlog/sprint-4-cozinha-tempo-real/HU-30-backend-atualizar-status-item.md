...
## Critérios de aceite
- [x] `PATCH /api/kitchen/items/:itemId` aceita `{ "kitchenStatus": "queued" | "preparing" | "ready" | "delivered" }` e atualiza o `order_item` correspondente.
- [x] Transições seguem a ordem `queued → preparing → ready → delivered`; uma transição para um status fora dessa sequência (ex.: `queued` direto para `delivered`) é rejeitada com erro de validação.
- [x] `GET /api/kitchen/tickets?orderId=` retorna os tickets e o status atual de cada item da comanda — usado como baseline/ressync pelo app (HU-33).
- [x] Atualizar um item inexistente retorna `404 NotFoundException`.
- [x] Cada atualização de status dispara o evento `item.status.changed` (consumido pela HU-31), mas o teste desta HU cobre apenas a persistência e a resposta REST.
...
- [x] Testes relevantes passando (casos de transição válida e inválida)
