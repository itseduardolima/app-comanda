...
## Critérios de aceite
- [ ] `GET /api/orders/:id` implementado, retornando a comanda com seus `order_item` (incluindo `menu_item_id`, `quantity`, `final_price`, `modifiers`, `kitchen_status` de cada item).
- [ ] Retorna 404 (`NotFoundException`) quando o `id` não existe.
- [ ] Total da comanda calculável a partir dos itens retornados (`quantity * final_price` de cada `order_item`), conforme regra de `02-modelo-de-dados.md`.
