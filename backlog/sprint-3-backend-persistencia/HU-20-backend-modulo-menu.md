...
## Critérios de aceite
- [ ] `MenuModule` criado com `controller` → `service` → Prisma, seguindo a camada padrão do repo.
- [ ] `GET /api/menu` retorna os itens do cardápio agrupados/anotados por categoria, incluindo `id`, `name`, `price`, `category` de cada `menu_item`.
- [ ] `GET /api/menu/categories` retorna a lista de categorias distintas.
- [ ] Ambas as rotas exigem `Authorization: Bearer <jwt>` (protegidas por guard), exceto se decidido o contrário para o cardápio (a confirmar com o time; se público, documentar a exceção).
- [ ] Resposta em JSON com campos em inglês, seguindo o contrato de `03-api-contrato.md`.
