...
## Critérios de aceite
- [x] Tela 1B (Escolher mesa) exibe grid de mesas mockadas com `table.status` (`free`/`occupied`).
- [x] Mesas **Livres** são selecionáveis; mesas **Ocupadas** são visualmente distintas (ex.: desabilitadas ou com indicação de que já têm comanda(s) aberta(s)).
- [x] Mesa ocupada pode ser tocada para ver as comandas já abertas nela (ex.: navega para lista de comandas daquela mesa), suportando o caso de **múltiplas comandas por mesa**.
- [x] Escolher uma mesa livre abre a comanda mockada (com o tipo `dine_in` e a mesa associada) e navega ao Cardápio (HU-07).
- [x] Textos de UI via i18n; `free`/`occupied` traduzidos conforme `.specs/06-glossario.md`.
