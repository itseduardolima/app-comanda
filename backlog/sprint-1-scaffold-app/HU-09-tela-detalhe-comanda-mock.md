...
## Critérios de aceite
- [ ] Tela 04 (Detalhe da comanda) lista os itens da comanda mockada com quantidade, modificadores e status individual (`kitchen_status`).
- [ ] Exibe o total da comanda (soma de `quantity * final_price` dos itens, sobre dados mockados).
- [ ] Ação **Adicionar** retorna ao Cardápio (HU-07) para incluir mais itens na mesma comanda.
- [ ] Ação **Fechar conta** está presente e, nesta fase mockada, apenas simula a mudança de `payment_status` para `paid` em memória (sem persistência real) e navega/atualiza a UI de acordo.
- [ ] Quando a mesa tem múltiplas comandas (ex.: João e Maria na Mesa 07), a tela deixa claro a qual comanda/pessoa aquele detalhe pertence.
- [ ] Textos de UI via i18n; enums traduzidos conforme `.specs/06-glossario.md`.
