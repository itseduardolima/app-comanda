...
## Critérios de aceite
- [ ] Tela `00B Acesso rápido por PIN` (mock do Sprint 1) permite digitar o PIN de 4 dígitos.
- [ ] Ao completar os 4 dígitos, chama `POST /api/auth/pin/verify` com `{ operatorId, pin }` automaticamente (sem precisar de botão extra "Confirmar").
- [ ] PIN incorreto exibe erro em português e limpa o campo para nova tentativa, sem navegar para outra tela.
- [ ] Sucesso entrega `accessToken` para a camada de sessão (HU-18) e redireciona para a tela `01 Comandas`.
- [ ] Teclado numérico exibido é adequado para entrada de PIN.
