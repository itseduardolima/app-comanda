...
## Critérios de aceite
- [ ] Tela `00B Acesso rápido por PIN` exibe uma opção "Trocar" usuário que leva de volta à tela `00 Login`.
...
- [ ] Existe uma ação de logout acessível a partir da tela `Perfil` (tab bar) que limpa a sessão ativa (token e dados do operador) e retorna à tela `00 Login`.
- [ ] Após logout, tentar acessar qualquer tela autenticada redireciona para `00 Login` (nenhuma tela protegida some visível "por trás").
