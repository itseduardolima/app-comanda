...
## Critérios de aceite
- [x] `POST /api/auth/login` aceita `{ "username": string }` e retorna `{ operatorId, pinSet }`.
- [x] Se o `username` não existe, retorna 404 (`NotFoundException`) com corpo no padrão `{ statusCode, message, error }`.
- [x] `pinSet` é `true` quando o operador já tem PIN cadastrado (`operator.pin_set = true`) e `false` no primeiro acesso.
- [x] Entrada validada via DTO com `class-validator` (username obrigatório, string não vazia).
- [x] Endpoint não exige `Authorization: Bearer` (é o único ponto de entrada anterior à autenticação).
- [x] Resposta não vaza `pin_hash` nem qualquer dado sensível do operador.
